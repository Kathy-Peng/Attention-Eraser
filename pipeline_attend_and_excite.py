...
class AttendAndExcitePipeline(StableDiffusionPipeline):
    ...
    Args:
        vae ([`AutoencoderKL`]nd decode images to and from latent representations.
    ...
    #given a prompt, this function returns a text embedding for the prompt
    def _encode_prompt(
        self,
        prompt,
        device,
    ...
    def _compute_max_attention_per_index(self,
        ...
        for i in indices_to_alter:
            ...
            sorting_index = torch.argsort(flattened_image)
            sorted_image = flattened_image[sorting_index]
            cur_sum = sum(sorted_image[-num_patches:])
            max_indices_list.append(cur_sum)
    ...
    def _compute_loss(max_attention_per_index: List[torch.Tensor], return_losses: bool = False) -> torch.Tensor:
        ...
        # loss = max(losses)
        losses = max_attention_per_index
        loss = sum(losses)
        if return_losses:
            return loss, losses
        else:
